...
**関数の既定値（`nextval()` / `now()` / `gen_random_uuid()`）は初期値に変換しない。**
実行のたびに値が変わり Java 側で写せないため。これらは `@Nullable` になる。
serial / identity の PK もこれに該当し、採番前を null で表せる。
...
`COLUMN_DEF` は `'X'::text` のようにクォートと型キャストを伴う。次の順で解釈する。

1. 末尾の `::型名` を取り除く
2. 前後のシングルクォートを取り除き、`''` を `'` に戻す
...
Spring Boot では推移的に入るが、依存の経路が変わると失われるため、
利用者には明示的な依存の追加を推奨する（README 参照）。
